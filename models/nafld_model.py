    ...
    def predict_risk(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.model is not None:
                ...
                # CatBoost çıktı sınıflarına göre (1=NAFL, 2=NASH vb.)
                if prediction == 1:
            ...
            # results.html artık impact_plot da bekliyor
            ...
            for feat, target in targets.items():
                ...
                if curr > target:
                    # MAFLD'de kilo ve enzim düşüşünün etkisini simüle et
                    weight = 20 if feat == 'Body Mass Index' else 12
                    impact = round(min(((curr - target) / curr) * weight, 28.0), 1)
                    if impact > 1.0:
                        xai_results['actionable_insights'].append({
                            'feature': feat.upper(), 'current': round(curr, 1), 
            ...
            try:
                ...
                if self.model is not None:
                    # CatBoost'un kendi önem değerlerini kullanmak en stabil yoldur
                    importances = self.model.get_feature_importance()
                    plt.barh([f.upper() for f in self.feature_names], importances, color='orange')
                else:
                    plt.barh([f.upper() for f in self.feature_names[:6]], [0.1]*6, color='gold')
                
                plt.title("1. Karar Ağırlığı (AI Teşhis Sebebi)")
                plt.xlabel("Parametre Önemi")
                plt.tight_layout()
                buf1 = io.BytesIO()
            ...
            try:
                ...
                if plot_data:
                    ...
                    plt.barh(names, vals, color='gold')
