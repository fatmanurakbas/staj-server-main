...
# --- YENİ EKLENEN: YEREL YAPAY ZEKA VE RAG KÜTÜPHANELERİ ---
...
# =====================================================================
# YEREL YAPAY ZEKA (FINE-TUNED) VE RAG SİSTEMİ BAŞLATMA
# =====================================================================
llm = None
search_model = None
rag_data = None
...
def init_local_ai():
    global llm, search_model, rag_data, chunks, embeddings
    import multiprocessing
    try:
        print("\n" + "="*50)
        print("Yerel Yapay Zeka ve RAG Sistemi Yükleniyor...")
        print("="*50)
        
        # 1. GGUF Modelini Yükle (HIZLANDIRILDI)
        model_path = "models/hepa_pro_model_ultra.gguf"
        if os.path.exists(model_path):
            # n_ctx (okuma kapasitesi) bilgisayarı yormasın diye 2048'e düşürüldü
            # n_threads bilgisayarın tüm çekirdeklerini kullanacak şekilde ayarlandı
            ...
            print(f"[OK] Llama Modeli Başarıyla Yüklendi ({threads} çekirdek kullanılıyor).")
        else:
            print(f"[UYARI] {model_path} bulunamadı!")

        # 2. RAG Vektör Arama Modelini Yükle
        search_model = SentenceTransformer('BAAI/bge-m3')
        print("[OK] Vektör Arama Modeli Yüklendi.")

        # 3. PDF Veritabanını Yükle
        if os.path.exists('models/rag_data.pkl'):
            with open('models/rag_data.pkl', 'rb') as f:
                rag_data = pickle.load(f)
                chunks = rag_data['chunks']
                embeddings = rag_data['embeddings']
            print("[OK] RAG Veritabanı (rag_data.pkl) Yüklendi.")
        else:
            print("[UYARI] rag_data.pkl bulunamadı!")
            
    except Exception as e:
        print(f"[HATA] Yerel AI başlatılırken sorun oluştu: {e}")

# Uygulama başlarken modelleri yükle
init_local_ai()

def get_rag_context(soru):
    """Verilen soruya en uygun PDF paragraflarını getirir"""
    if search_model is None or embeddings is None or len(chunks) == 0:
        return "RAG veritabanı aktif değil."
    try:
        query_vec = search_model.encode([soru])
        sims = cosine_similarity(query_vec, embeddings)[0]
        # HIZLANDIRMA: Bilgisayarı yormamak için 5 paragraf yerine en iyi 2 paragrafı çekiyoruz
...
def format_model_name(model_path: str) -> str:
    ...
    # Yeni modellere göre özel isimlendirme
    if model_name == 'llama-3.1-8b-instruct:free':
        return 'Llama 3.1 (Ücretsiz)'
    elif model_name == 'gemma-2-9b-it:free':
        return 'Gemma 2 (Ücretsiz)'
    elif model_name == 'gpt-4o' or model_name == 'gpt-3.5-turbo':
    ...
    try:
        ...
        def make_serializable(obj):
            if isinstance(obj, dict):
                ...
                return obj
        # --- COOKIE ŞİŞMESİNİ ÖNLEME DÜZELTMESİ ---
        # Verileri serialize et
        patient_session = make_serializable(patient_data)
        results_session = make_serializable(results)
        scores_session = make_serializable(traditional_scores)
        interp_session = make_serializable(score_interpretations)
        
        # XAI'daki devasa resim grafiklerini (Base64) session'dan sil (Cookie sınırını aşmamak için)
        for d_key in results_session:
            if isinstance(results_session[d_key], dict) and 'xai' in results_session[d_key] and isinstance(results_session[d_key]['xai'], dict):
                results_session[d_key]['xai'].pop('shap_plot', None)
                results_session[d_key]['xai'].pop('impact_plot', None)
        
        # Temizlenmiş verileri session'a kaydet
        session['patient_data'] = patient_session
        session['results'] = results_session
        session['traditional_scores'] = scores_session
        session['score_interpretations'] = interp_session
        # ----------------------------------------
...
# =====================================================================
# YENİ EKLENEN: YEREL CHATBOT (RAG + FINE-TUNING) ENDPOINT'İ
# =====================================================================
@app.route('/ask_rag', methods=['POST'])
@login_required
def ask_rag():
    """Local LLM and RAG based chatbot endpoint"""
    print("\n" + "!"*40)
    print("YENİ BİR CHATBOT İSTEĞİ GELDİ!")
    print("!"*40)
    
    if llm is None or search_model is None or embeddings is None:
        return jsonify({'success': False, 'error': 'Yerel AI sistemi şu an aktif değil. Lütfen sunucu yöneticisine başvurun.'}), 503
    ...
    try:
        ...
        # 1. RAG ile PDF'lerden context çekme
        print("1/4: PDF dökümanları (RAG) taranıyor...")
        context = get_rag_context(soru)
        
        # 2. Hasta değerlerini metne dönüştürme
        hasta_ozeti = f"""
        Yaş: {patient_data.get('age', 'Bilinmiyor')}
        Cinsiyet: {'Kadın' if patient_data.get('gender') == 1 else 'Erkek' if patient_data.get('gender') == 0 else 'Bilinmiyor'}
        BMI: {patient_data.get('bmi', 'Bilinmiyor')}
        AST: {patient_data.get('ast', 'Bilinmiyor')}
        ALT: {patient_data.get('alt', 'Bilinmiyor')}
        Platelet: {patient_data.get('trombosit', 'Bilinmiyor')}
        """
        
        # 3. Modele gönderilecek Süper Prompt
        print("2/4: Yapay zeka promptu hazırlandı.")
 # 3. Modele gönderilecek Süper Prompt (GÜNCELLENDİ)
        prompt = f"""<|start_header_id|>system<|end_header_id|>
Sen uluslararası düzeyde uzmanlaşmış Kıdemli bir Hepatoloji Profesörüsün. Sana sunulan "REHBER DÖKÜMANLARI" ve "HASTA VERİLERİ" bağlamında klinik karar vermelisin.
Kurallar:
1. Akademik bir doktor üslubu kullan.
2. Soruya yanıt verirken ÖNCE "REHBER DÖKÜMANLARI" kullan. Eğer aradığın spesifik cevap bu dökümanlarda HİÇ YOKSA, soruyu papağan gibi tekrar etme! Doğrudan şunu söyle: "Yüklenen güncel rehber dökümanlarında bu konu hakkında spesifik bir bilgi bulunmamaktadır."
3. Eğer dökümanda yoksa genel tıbbi bilgini SADECE "Genel tıbbi literatüre göre..." diyerek ekleyebilirsin ama kesinlikle dökümandan alınmış gibi gösterme.

REHBER DÖKÜMANLARI:
{context}

HASTA VERİLERİ:
{hasta_ozeti.strip()}<|eot_id|><|start_header_id|>user<|end_header_id|>
Klinik Soru: {soru}<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""

        # 4. Yerel modelden cevap üretimi
        print("3/4: Llama 3 Modeline gönderildi... (LÜTFEN BEKLEYİN, İŞLEMCİ İLE 1-3 DAKİKA SÜREBİLİR!)")
        output = llm(
            prompt, 
            max_tokens=256,   # HIZLANDIRMA: Cevabı kısa kesmesi için 256'ya düşürüldü
            temperature=0.1,  
            top_p=0.9,
            stop=["<|eot_id|>", "<|end_of_text|>", "User:", "Soru:"],
            echo=False
        )
        
        cevap = output['choices'][0]['text'].strip()
        print("4/4: Yapay zeka cevabı başarıyla üretti!")
        print(f"Cevap Önizlemesi: {cevap[:50]}...")
        print("!"*40 + "\n")
        
        return jsonify({'success': True, 'answer': cevap})
        
    except Exception as e:
        print(f"\n[HATA] RAG Endpoint Çöktü: {str(e)}\n")
...
def doctor_assessment():
    try:
        ...
        doctor_map = {
            'smith': {'name': 'Dr. GPT-Mini (Hızlı)', 'model': 'gpt-3.5-turbo', 'provider': 'openai'},
            'johnson': {'name': 'Dr. GPT-4o (Uzman)', 'model': 'gpt-4o', 'provider': 'openai'},
            'brown': {'name': 'Dr. Gemini', 'model': 'gemini-2.5-flash', 'provider': 'google'}
        }
...
def process_document():
    """Process uploaded medical document using OpenAI GPT-4o"""
    import base64
    import PyPDF2
    from io import BytesIO
...
# OpenAI'ın KESİNLİKLE kullanması gereken format:
        expected_format = """
        SADECE bu JSON yapısını kullan. Anahtar isimlerini (İngilizce olanları) asla değiştirme. Bulamadığın değere null yaz:
        {
            "age": "yaş (sayı)",
            "gender": "Erkek ise 1, Kadın ise 2",
            "bmi": "VKİ değeri (sayı)",
            "obesity": "Obez ise 1, değilse 0",
            "ast": "AST değeri (sayı)",
            "alt": "ALT değeri (sayı)",
            "trombosit": "Trombosit/PLT değeri (sayı)",
            "albumin": "Albumin değeri (sayı)",
            "inr": "INR değeri (sayı)",
            "total_bilirubin": "Total Bilirubin değeri (sayı)",
            "direct_bilirubin": "Direkt Bilirubin değeri (sayı)",
            "creatinine": "Kreatinin değeri (sayı)",
            "alp": "ALP değeri (sayı)",
            "afp": "AFP değeri (sayı)"
        }
        """

        # 1. DURUM: EĞER DOSYA PDF İSE
        if file_ext == 'pdf':
            try:
                ...
                messages = [
                    {"role": "system", "content": expected_format},
                    {"role": "user", "content": f"{ocr_prompt}\n\nİşte belgenin metni:\n{document_text}"}
                ]
            except Exception as e:
                return jsonify({'success': False, 'error': f'PDF okuma hatası: {str(e)}'})
                
        # 2. DURUM: EĞER DOSYA RESİM İSE (JPG, PNG)
            ...
            return jsonify({'success': False, 'error': 'Lütfen sadece PDF, JPG veya PNG formatında dosya yükleyin.'})
        ...
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=messages,
            response_format={ "type": "json_object" }, # Çıktının KESİNLİKLE JSON olmasını zorlar
            max_tokens=1500,
            temperature=0.1 # Halüsinasyon görmemesi için düşük sıcaklık
        )
        
        # OpenAI'dan gelen yanıtı al (Artık regex ile temizlememize gerek yok, doğrudan JSON gelir)
        response_text = response.choices[0].message.content
        extracted_data = json.loads(response_text)
        
        print(f"OPENAI DOCUMENT EXTRACTION SUCCESS:")
        ...
        return jsonify({'success': False, 'error': 'Yapay zeka veriyi JSON formatına çeviremedi.'})
    except Exception as e:
...
def get_ai_assessment():
    """Get AI doctor assessment enriched with XAI (Explainable AI) insights"""
    try:
        # İstekten seçilen doktoru al
        data = request.get_json()
        selected_doctor = data.get('doctor', '')
        lang = session.get('language', 'tr')
        
        # Oturum verilerini geri al
        patient_data = session.get('patient_data', {})
        results = session.get('results', {})
        traditional_scores = session.get('traditional_scores', {})
        ...
        if not patient_data or not results:
            return jsonify({
                'success': False, 
                'error': 'Oturum verisi bulunamadı. Lütfen önce riskleri hesaplayın.'
            })
    
# YENİ DOKTOR HARİTASI (OpenRouter Ücretsizler + Kendi OpenAI Key'iniz)
# DOĞRUDAN ORİJİNAL API'LER KULLANILACAK
        doctor_map = {
            'smith': {'name': 'Dr. GPT-Mini (Hızlı)', 'model': 'gpt-3.5-turbo', 'provider': 'openai'},
            'johnson': {'name': 'Dr. GPT-4o (Uzman)', 'model': 'gpt-4o', 'provider': 'openai'},
            'brown': {'name': 'Dr. Gemini', 'model': 'gemini-2.5-flash', 'provider': 'google'}
        ...
        if selected_doctor not in doctor_map:
            return jsonify({'success': False, 'error': 'Geçersiz doktor seçimi.'})
        
        doctor_info = doctor_map[selected_doctor]
        
        # --- XAI (AÇIKLANABİLİRLİK) VERİLERİNİ HAZIRLA ---
        xai_data_for_prompt = ""
        cirrhosis_res = results.get('cirrhosis', {})
        
        if 'xai' in cirrhosis_res and cirrhosis_res['xai']:
            insights = cirrhosis_res['xai'].get('actionable_insights', [])
            if insights:
                xai_data_for_prompt = "\n\n### YAPAY ZEKA MODEL ANALİZİ (XAI):\n"
                xai_data_for_prompt += "Matematiksel modelimiz (SHAP), bu hasta için şu iyileştirmelerin riski doğrudan düşüreceğini saptadı:\n"
                for i in insights:
                    xai_data_for_prompt += f"- {i['feature'].upper()}: Şu an {i['current']} birim. {i['target']} birimin altına düşerse risk %{i['impact']} oranında azalacaktır.\n"
                xai_data_for_prompt += "\nLütfen bu verileri kullanarak hastaya somut ve eyleme geçirilebilir tıbbi önerilerde bulun."

        with open('PROMPT', 'r', encoding='utf-8') as f:
            prompt_template = f.read()
        
        patient_data_str = '\n'.join([f"{k}: {v}" for k, v in patient_data.items()])
        patient_data_str += xai_data_for_prompt 
        traditional_scores_str = '\n'.join([f"{k}: {v}" for k, v in traditional_scores.items()])
        ...
        print(f"\n{'='*60}")
        print(f"XAI DESTEKLİ AI DOKTOR TAVSİYESİ - {doctor_info['name']}")
        print(f"{'='*60}")
        
       # --- HANGİ API'YE GİDECEĞİMİZİ SEÇİYORUZ ---
        try:
            if doctor_info['provider'] == 'openai':
                # Sizin OpenAI API'nize bağlanır
            ...
            elif doctor_info['provider'] == 'google':
                # Sizin Gemini API'nize bağlanır
            ...
            print(f"API Hatası ({doctor_info['provider']}): {str(api_error)}")
            return jsonify({
                'success': False,
                'error': f"{doctor_info['name']} ile bağlantı kurulamadı: {str(api_error)}"
            })
            
        ai_assessment_title = f"{doctor_info['name']} XAI-Destekli Analiz"
        
        # Markdown'ı HTML'e çevir
        ai_assessment_html = markdown.markdown(ai_assessment)
        
        return jsonify({
            'success': True,
            'assessment': ai_assessment_html,
            'title': ai_assessment_title
        })
        
    except Exception as e:
        print(f"Error getting AI assessment with XAI: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'AI doktor değerlendirmesi alınırken hata oluştu: {str(e)}'
        })
